...
TEST(MessageRelayTest, SanitizeField) {
    EXPECT_EQ(InputValidator::sanitize_field("valid_string"), "valid_string");
    EXPECT_EQ(InputValidator::sanitize_field("string with \" quotes"), "string with   quotes");
    
    std::string long_string(1000, 'a');
    std::string sanitized = InputValidator::sanitize_field(long_string, 100);
...
TEST(MessageRelayTest, MessageSizeConfig) {
